...
import { fetchTeachersList } from "../../redux/slices/adminSlice";
...
const TeacherList = () => {
  ...
  const teachers = useSelector((state: RootState) => state.admin.teachersList);
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof typeof dummyTeachers[0] | null; direction: 'asc' | 'desc' }>({ key: null, direction: 'asc' });
  ...
  useEffect(() => {
    dispatch(fetchTeachersList() as any);
  }, [dispatch]);

  const filteredTeachers = useMemo(() => {
    const dataToFilter = ((teachers as any) as Teacher[]) || dummyTeachers;
    if (searchQuery.trim() === '') {
      return dataToFilter;
    } else {
      return dataToFilter.filter(
        (item) =>
          ...
          item.addDate?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
  }, [searchQuery, teachers]);

  const sortedData = useMemo(() => {
    const sortableItems = [...filteredTeachers];
  ...
  const handleSort = (key: keyof typeof dummyTeachers[0]) => {
  ...
  const renderTableHeader = () => (
    <View style={styles.headerRow}>
      ...
      <TouchableOpacity style={[styles.headerCell, { width: columnWidths.subject }]} onPress={() => handleSort('subject')}>
        <Text style={styles.headerText}>
          SUBJECT {" "}
      ...
      <TouchableOpacity style={[styles.headerCell, { width: columnWidths.addDate }]} onPress={() => handleSort('addDate')}>
        <Text style={styles.headerText}>
          ADD DATE {" "}
  ...
  const renderTableRow = ({ item, index }: { item: any; index: number }) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.cell, styles.cellText, { width: columnWidths.subject }]}>{item.subject}</Text>
      <Text style={[styles.cell, styles.cellText, { width: columnWidths.addDate }]}>{formatDate(item.addDate)}</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.tableContainer}>
          <Text style={styles.tableTitle}>TEACHERS LIST</Text>
